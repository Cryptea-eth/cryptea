...
const VerifyHash = () => {
  ...
  useEffect(() => {
    if (hash !== undefined) {
      http
        .post(`/login/magic?tz=${window.jstz.determine().name()}`, {
          magic: String(hash),
        })
        .then((userx) => {
          const {
    ...
    <div className="h-screen">
      ...
      <div className="w-full h-[calc(100vh-100px)] flex flex-col justify-center items-center px-5 my-8">
        ...
        <span className="text-[#7e7e7e] block px-4 font-semibold text-lg mx-auto mt-5">
          ...
          checking your internet access and reload the page, click the button
          below to request new link.
        </span>

        <Link href={"/magic"}>
          <a className="flex items-center justify-center mt-3">
            <Button className="hover:!bg-[#4a168e] !transition-all !delay-500 !text-sm !capitalize !rounded-lg !bg-[#8036de] !text-white !font-semibold !py-3 !px-4">
